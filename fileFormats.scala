...
val student_name = "Muhammad Hassan"
...
val df_parquet = spark.read.schema(customSchema).parquet(source_parquet_folder)
...
val df_new_rows = df_csv
                  ...
                  .withColumn("row_num", row_number().over(Window.partitionBy("City").orderBy(col("Start_Time").desc)))
                  .filter(col("row_num") <= more_rows_count)
                  .drop("row_num")
...
// MAGIC
// MAGIC I got similar results to the output shared below. The nuber of columns changed and also the data type of each column changed to string type due to which the values in Temperature_F column also got changed. 
// MAGIC
// MAGIC *What is your explanation/guess for why the CSV seems broken and the schema cannot be inferred anymore?*
// MAGIC
// MAGIC CSV files generally lack schema definition. When spark try to append data with a different schema, it may not be able to explicitely enforce the schema and hence the extra columns are not handeled correctly resulting in a broken CSV file.
// MAGIC
// MAGIC
...
// MAGIC
// MAGIC I got my results similar to Parquet case 1.
// MAGIC
...
// MAGIC
// MAGIC Parquet files store data with strict typing and hence we can see that datatypes remain the same but the mumber of columns did not change as Parquet files store data with column ordering and hence change in schema was not accepted.
// MAGIC
...
val df_delta = spark.read.format("delta").load(source_delta_folder)
...
val modified_delta_df = spark.read.format("delta").load(target_file_delta)
...
deltatable.as("target")
  .merge(df_delta_update.as("source"), "target.ID = source.ID")
  .whenMatched
  .updateAll()
  .whenNotMatched
  .insertAll()
